...
import {nms} from './engine.js?v=23';
ort.env.wasm.wasmPaths=new URL('./vendor/',self.location.href).href;
ort.env.wasm.numThreads=1;
let session,sessionSize;
self.onmessage=async({data})=>{
  let tensor,outputs;
  try{
    const size=data.size===640?640:960;
    if(session&&sessionSize!==size){await session.release();session=null;}
    if(!session){self.postMessage({id:data.id,status:'Loading the hold detector (12 MB)…'});session=await ort.InferenceSession.create(`./models/holds-${size}.onnx`,{executionProviders:['wasm'],graphOptimizationLevel:'all'});sessionSize=size;}
    self.postMessage({id:data.id,status:'Finding holds on your wall…'});
    const {pixels,width,height}=data,scale=Math.min(size/width,size/height),rw=Math.round(width*scale),rh=Math.round(height*scale),px=Math.floor((size-rw)/2),py=Math.floor((size-rh)/2);
    ...
    outputs=await session.run({images:tensor});const output=outputs[session.outputNames[0]],d=output.data,n=output.dims[2],boxes=[];
    for(let i=0;i<n;i++){
    ...
    self.postMessage({id:data.id,boxes:nms(boxes),size});
  }catch(e){self.postMessage({id:data.id,error:e.message||'Detector unavailable'});}
  finally{tensor?.dispose();if(outputs)Object.values(outputs).forEach(o=>o.dispose());}
};
