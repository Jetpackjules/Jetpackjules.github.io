(()=>{
 ...
 const pairRate=1.3;
 const pairNote='Recorded live · 1.3× playback. The two recordings are aligned for viewing.';
 const setPairRate=()=>{for(const video of [main,other]){video.defaultPlaybackRate=pairRate;video.playbackRate=pairRate;}};
 setPairRate();
 ...
 const start=async()=>{
   ...
   else status.textContent=pairNote;
    update();
  };
 ...
 for(const video of [main,other])video.addEventListener('loadedmetadata',setPairRate);
