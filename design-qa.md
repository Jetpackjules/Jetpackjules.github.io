...
- Both physical-session clips autoplay muted, loop inline and retain synchronization. Both source durations are 17.2 seconds. The final page has no paired controls, seek bar or playback note. Other project-page videos retain manual playback.
...
- Verified 1.3 playbackRate and defaultPlaybackRate on only the two top physical clips. The full narrated video and both simulation players remain at 1.0 and do not autoplay. Playback resumes automatically after the page becomes visible again.
- The portfolio thumbnail uses the existing gallery video component with autoplay, muted playback and looping. Its 5.733-second H.264 file encodes the full physical alignment/release clip at 3× speed, at 640×360 and 30 fps. It has no audio, passes full decode, and retains the original still as its poster.
...
Passed for the requested design adaptation, responsive layout, automatic paired playback, animated gallery thumbnail, media loading, and production build. The webcam and physical leader still require device testing by the user; they are features of the local demo rather than this static page.
